...
const AccordionItem = ({ title, icon: Icon, children, defaultOpen = false, className = "", contentClassName = "p-4" }) => {
...
const NestedAccordion = ({ title, count, children }) => {
...
function App() {
  const [signData, setSignData] = useState({
    ...
    icon: 'AlertOctagon',
    layout: 'standard',
    mainTextSize: 150, // Scaled for A4 (1123px width)
    subTextSize: 75,   // Scaled for A4
    borderWidth: 12,   // Scaled for A4
    showShadow: true,
    ...
    headerHeight: 25,
    iconColor: '#000000',
    headerTextSize: 0 // 0 means auto/default
  });
  
  const [templates, setTemplates] = useState([]);
  const [searchTerm, setSearchTerm] = useState('');
  
  // Organized Template State
  const [groupedTemplates, setGroupedTemplates] = useState({});
  
  // Sign Dimensions & Position State
  const [signSize, setSignSize] = useState({ width: 1123, height: 794 }); // Default to A4 Landscape
  const [signPosition, setSignPosition] = useState({ x: 50, y: 50 });
  const [zoom, setZoom] = useState(0.6); // Start zoomed out to fit
  const [paperFormat, setPaperFormat] = useState('a4');
  const [showGrid, setShowGrid] = useState(true);

  // Custom Elements (Images, Icons, Shapes)
  const [extraElements, setExtraElements] = useState([]);
  const [selectedElementIds, setSelectedElementIds] = useState([]);
  const [selectedElementId, setSelectedElementId] = useState(null); // Primary selection for props
  ...
  const centerSign = useCallback(() => {
    ...
    const newZoom = Math.min(scaleX, scaleY, 1.2); // Cap zoom at 120%
    
    setZoom(Math.max(newZoom, 0.1)); // Min zoom 10%
  ...
  useEffect(() => {
     // Auto-center when paper size changes
     setTimeout(centerSign, 100);
  }, [signSize, centerSign]);

  const [printOptions, setPrintOptions] = useState({ bleed: false, cropMarks: false });
  ...
  const saveToHistory = useCallback(() => {
    ...
    setHistoryIndex(prev => Math.min(prev + 1, 49)); // Adjust index logic if shifted
  }, [signData, signSize, signPosition, extraElements, historyIndex]);

  // Use debounce for text/slider inputs to avoid spamming history
  useEffect(() => {
    ...
    fetch('http://localhost:3001/api/templates')
      ...
      .then(data => {
        if (Array.isArray(data)) {
          ...
          const grouped = data.reduce((acc, tmpl) => {
  ...
  const confirmSaveTemplate = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Update grouped
        const cat = 'My Saved Designs';
        setGroupedTemplates(prev => {
           const next = { ...prev };
           if (!next[newTmpl.type]) next[newTmpl.type] = {};
           if (!next[newTmpl.type][cat]) next[newTmpl.type][cat] = [];
           next[newTmpl.type][cat].push(newTmpl);
           return next;
        });
  ...
  const handleElementChange = (id, newProps, isResizing) => {
    if (id === 'main-sign') {
  ...
  const getFilteredTemplates = () => {
    // Separate saved designs first
    const saved = templates.filter(t => 
      (t.id && t.id.toString().startsWith('custom-')) || t.category === 'My Saved Designs'
    );
    
    // Standard templates are everything else
    const standard = templates.filter(t => 
      !((t.id && t.id.toString().startsWith('custom-')) || t.category === 'My Saved Designs')
    );

    if (!searchTerm) {
        // Regroup standard templates
        const groupedStandard = standard.reduce((acc, tmpl) => {
            if (!acc[tmpl.type]) acc[tmpl.type] = {};
            if (!acc[tmpl.type][tmpl.category]) acc[tmpl.type][tmpl.category] = [];
            acc[tmpl.type][tmpl.category].push(tmpl);
            return acc;
    ...
    // Filter and group standard
    const filteredGrouped = {};
    // Temporarily group standard first to make filtering easier or just filter then group
  ...
  const handleLayerReorder = (direction) => {
  ...
  const handleBatchDownload = async () => {
     ...
     const originalSignData = JSON.parse(JSON.stringify(signData));
     ...
     try {
        ...
        for (let i = 0; i < selectedBatchIds.length; i++) {
            ...
            let tmpl = null;
            Object.values(groupedTemplates).forEach(catObj => {
                Object.values(catObj).forEach(list => {
            ...
            if (tmpl) {
                ...
                if (node) {
                     ...
                     const dataUrl = await htmlToImage.toPng(node, { 
                        ...
                        width: totalWidth, 
                        height: totalHeight,
                        style: { transform: 'none', display: 'flex', opacity: 1 } 
                     });
        ...
        pdf.save(`batch-signs-${Date.now()}.pdf`);
        
     } catch (e) {
        console.error("Batch download failed", e);
        alert("Batch download failed.");
     } finally {
        setSignData(originalSignData);
  ...
  const handleDeleteTemplate = async (e, tmpl) => {
    ...
    try {
        const res = await fetch(`http://localhost:3001/api/templates/${tmpl.id}`, {
            method: 'DELETE'
        });

        if (res.ok) {
            setTemplates(prev => {
                const newTemplates = prev.filter(t => t.id !== tmpl.id);
                
                // Re-group
                const grouped = newTemplates.reduce((acc, t) => {
                    if (!acc[t.type]) acc[t.type] = {};
                    const cat = t.category || 'Other'; 
                    if (!acc[t.type][cat]) acc[t.type][cat] = [];
                    acc[t.type][cat].push(t);
                    return acc;
                }, {});
                setGroupedTemplates(grouped);
  ...
  return (
    <div className="flex h-screen bg-gray-100 font-sans overflow-hidden flex-col">
      ...
      {/* --- Top Toolbar --- */}
      <header className="h-14 bg-slate-950 border-b border-slate-800 flex items-center justify-between px-4 z-30 shadow-md" id="app-header">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            ...
            <AccordionItem title="Template Library" icon={FileText} defaultOpen={false} contentClassName="p-0">
              <div className="p-4 relative">
                ...
                <input 
                  type="text" 
                  placeholder="Search 10,000+ hazards..." 
                  value={searchTerm}
                        ...
                        <div className="pl-4 pr-2 py-1 space-y-1">
                          {displayTemplates[type][category].map(tmpl => (
                            ...
                            >
                               ...
                               {/* Delete Button for Custom/Saved Templates - NOW HANDLED IN SEPARATE SECTION, BUT KEEPING FOR FALLBACK IF MIXED */}
             {((category === 'My Saved Designs' || category === 'Custom' || (tmpl.id && tmpl.id.toString().startsWith('custom-'))) && !savedTemplates.some(t => t.id === tmpl.id)) && (
                 <div
                     onClick={(e) => handleDeleteTemplate(e, tmpl)}
                     className="opacity-0 group-hover/btn:opacity-100 p-1 text-slate-500 hover:text-red-400 hover:bg-slate-800 rounded transition-all absolute right-1 top-1.5 z-10 cursor-pointer"
                     title="Delete Template"
            ...
            <AccordionItem title="Layers & Elements" icon={List} defaultOpen={true}>
              <div className="space-y-1 max-h-40 overflow-y-auto custom-scrollbar pr-1">
                ...
                {[...extraElements].reverse().map((el, idx) => (
                  <div 
                    key={el.id}
            ...
            <div style={{
               ...
               height: signSize.height
                ...
                {extraElements.map(el => {
                  const IconComp = el.type === 'icon' && iconMap[el.iconName];
                      ...
                      {[
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            // Scale Font Sizes to maintain relative look
                            setSignData(prev => ({
                              ...prev,
                            ...
                            // Scale Extra Elements (Icons/Images)
                            setExtraElements(prev => prev.map(el => ({
                              ...el,
                              x: el.x * scale,
                              y: el.y * scale,
      ...
      {/* Hidden Print Container for PDF Export (Clean, No Zoom, No Grid) */}
      <div ref={printRef} style={{
          ...
          pointerEvents: 'none',
          width: signSize.width + (printOptions.bleed ? 40 : 0) + (printOptions.cropMarks ? 60 : 0), // Extra space for bleed/crop
          ...
          <div style={{ 
            position: 'relative', 
            width: signSize.width + (printOptions.bleed ? 24 : 0), // 3mm bleed approx 12px each side -> 24px total
            height: signSize.height + (printOptions.bleed ? 24 : 0),
            backgroundColor: 'white', // Background for bleed area
            ...
            <div style={{
              ...
              height: signSize.height,
              position: 'relative',
              overflow: printOptions.bleed ? 'visible' : 'hidden' // Allow content to bleed if enabled
            }}>
                {/* Main Sign Background - Always at 0,0 in this container */}
                <div style={{ width: '100%', height: '100%', position: 'absolute', top: 0, left: 0, zIndex: 0 }}>
                   <SafetySign {...signData} />
                </div>

                {/* Extra Elements - Positioned relative to the sign origin */}
                {extraElements.map(el => {
                   const IconComp = el.type === 'icon' && iconMap[el.iconName];
                   const relX = el.x - signPosition.x;
                   const relY = el.y - signPosition.y;
